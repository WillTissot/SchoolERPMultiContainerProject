...
namespace SchoolERP.MVC.UI.Controllers
{
    public class ApplicationUsersController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                var url = "http://Students.Services/api/Student"; //to be changed 
        ...
        public async Task<IActionResult> Details(Guid? id)
        {
            ...
            try
            {
                var url = "http://Students.Services/api/Student/" + id.ToString(); //to be changed 
                var restClient = new RestClient(url);
                var request = new RestRequest(url, RestSharp.Method.Get);
                RestResponse response = await restClient.ExecuteAsync(request);
                var output = response.Content;
        ...
        public async Task<IActionResult> Edit(Guid? id)
        {
            ...
            try
            {
                var url = "http://Students.Services/api/Student/" + id.ToString();  
                var restClient = new RestClient(url);
                var request = new RestRequest(url, RestSharp.Method.Get);
                RestResponse response = await restClient.ExecuteAsync(request);
                var output = response.Content;
        ...
        public async Task<ActionResult> InsertGrades(List<StudentCourseVM> studentCourse)
        {
            ...
            try
            {
                var url = "http://Students.Services/api/Student/Grade"; 
                var restClient = new RestClient(url);
                var request = new RestRequest(url, RestSharp.Method.Post);
